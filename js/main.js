...
document.addEventListener("DOMContentLoaded",()=>{
    ...
    const update=()=>{
        ...
        if(status=="play"){
            drawPlayer();
            //1フレームにかかった時間(deltaTime)をelapsedTimeに加算
        ...
        if(status == "end"){
            ...
            drawText("Click here to retry",width/2,height/2+30,30,"red");
        }else if(status=="ready"){
            drawText("Click here to start",width/2,height/2+30,30,"red");
        }
        //ディスプレイのリフレッシュレートのタイミングで実行される（基本1秒間に60回）
